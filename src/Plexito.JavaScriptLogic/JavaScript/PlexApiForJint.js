function GetDevices_jint(xhr) {
    ...
    return JSON.stringify(xmlToJson_jint(xmlDoc.rootElement.childElements[0]));
}

function xmlToJson_jint(xml) {
    ...
    for (var i = 0; i < xml.childElements.length; i++) {
        ...
        if (typeof (obj[nodeName]) == "undefined") {
            obj[nodeName] = xmlToJson_jint(item);
        } else {
            ...
            obj[nodeName].push(xmlToJson_jint(item));
        }
    }
    return obj;
};
